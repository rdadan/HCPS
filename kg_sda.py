...
def readdata_SDA_ML1M(root_path, serNum, batch_size, use_cuda=False, test=False):
    ...
    for i in range(serNum):
        ...
        step = 50
        batch_size = len(sHotTrainData[i]) // step
        ...
        batchUser = len(sColdTrainData[i][0]) // step
        batchAD = len(sColdTrainData[i][1]) // step
        ...
        print('!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! step:', len(sHotTrainData[i]), len(coldU), len(coldAD))
...
class WideDeep(nn.Module):
    ...
    def forward(self, x):
        ...
        # Wide
        wide_out = self.linear(inputs)
        # Deep
        dnn_input = torch.cat(inputs_embeds, dim=-1)
...
def serverMetaTrainUorA(epoch, net, hot, cold, lr):
    oriTheta, metaGradientU, metaGradientA = {}, {}, {}
    initStateDict(oriTheta, net)
    initStateDict(metaGradientU, net)
    initStateDict(metaGradientA, net)
    tasks = len(hot)
    ...
    for epo in range(1, epoch + 1):
        ...
        for i, coldtData in enumerate(list(cold[0])):
            ...
            updataNetWeight(net, theta_matrix[i])
            auc, _ = net.train_step(coldX, coldY)
            getMetaGradient(metaGradientU, net)
        for i, coldtData in enumerate(list(cold[1])):
            coldX, coldY = coldtData[0], coldtData[1]
            updataNetWeight(net, theta_matrix[i])
            auc, _ = net.train_step(coldX, coldY)
            getMetaGradient(metaGradientA, net)
        # 更新初始的oriTheta
        for name, wight in oriTheta.items():
            gradient = (metaGradientU[name] + metaGradientA[name]) / tasks
            oriTheta[name] = wight - lr * gradient
...
def sda_fed_torchkeras(Epoch, nets, modleType, fed_type, trainColIdx,
    ...
    for epo in range(1, Epoch + 1):
        ...
        if fed_type == 'FedME':
            ...
            # 训练 user (localEpo, nets, deviceTrainData, deviceServerID, modleType, lr)
            ...
            for i, (net, hot, cold, test) in enumerate(zip(nets, sHotTrainData, sColdTrainData, sColdTestData)):
                serverMetaTrainUorA(2, net, hot, cold, lr)
        ...
        if fed_type == 'FedEmb':
            ...
            deviceNets = deviceLocalTrain(nets, userCol, deviceTrainData, deviceServerID, modleType, lr)
            serverAggregateUserEmb(nets, deviceNets, deviceUserServerList, serverDeviceID, userCol)
            _, best_nets, _ = trainNet(1, nets, modleType, sColdTrainData, lr, best_nets,
                                       trainColIdx, printAuc=True)
            print(" ------------------------------------------- fed phrase eval")
...
def loadPreTrain(modleType, sHotTestData, sColdTestData, trainColIdx, fea_dims, use_cuda, PATH=None):
    print("load PreTrain")
    preNet = []
    for i in range(len(sHotTestData)):
        name = modleType + str(i) + '.net'
...
def train_sda():
    ...
    fea_dims, sHotTrainData, sHotTestData, sColdTrainData, sColdTestData, sHotForColdData, \
    deviceTrainData, deviceServerID, deviceUserServerList, serverDeviceID, aData = \
        readdata_SDA_ML1M(path, serNum, batch_size=500, test=test)
