...
# item_cols = ['movie_id', 'movie_title', 'genre']
# rating_cols = ['user_id', 'movie_id', 'rating', 'timestamp']
# rating_cols=['user_id', 'movie_id', 'movie_title', 'genre', 'timestamp', 'gender', 'age', 'occupation', 'zip_code', 'rating']
...
def agg_item_emb(nets, aData, use_cuda, strategy):
    ...
    v_features = ['movie_id', 'movie_title', 'genre', 'serverIdx']
    fea = v_features[0]
    movie_id, server_id = aData['movie_id'], aData[v_features[-1]]
    # for fea in v_features[:2]:
    emb_name = "net.embed_layers.embed_" + fea + ".weight"
...
def getColdUserAndAD(rating):
    ad = rating['movie_id'].value_counts().to_frame()
    coldAdID = ad.loc[ad.values < 15].index.tolist()
    coldAdRating = rating.loc[rating['movie_id'].isin(coldAdID)]

    user = rating['user_id'].value_counts().to_frame()
    coldUserID = user.loc[user.values < 30].index.tolist()
    coldUserRating = rating.loc[rating['user_id'].isin(coldUserID)]

    coldRating = pd.concat([coldUserRating, coldAdRating], axis=0)
    coldRating = coldRating.drop_duplicates()
    return coldRating, coldUserRating, coldAdRating


def getColdAndHotRating(df_rating):
    total = len(df_rating)
    coldRating, coldUserRating, coldAdRating = getColdUserAndAD(df_rating)
    hotRating = df_rating.drop(coldRating.index)
    coldRating = {'coldUser': coldUserRating, 'coldAD': coldAdRating}
    sum = len(hotRating) + len(coldRating)
    if total == sum:
        print('hot+cold == rating')
    return coldRating, hotRating

def getServerData(data, serverDeviceID, sHotForColdData, sHotTrainData, sHotTestData, sColdTrainData, sColdTestData):
    serverDeviceID.append(data['occupation'].unique())
    coldRating, hotRating = getColdAndHotRating(data)
    hotforcold = hotRating[:int(0.2 * len(hotRating))]
...
def readdata_SDA_ML1M(root_path, serNum, batch_size, use_cuda=False, test=False):
    ...
    sHotForColdData = []
    serverDeviceID = []
    allColdTrain, allColdTest = [], []
    nHot, nHotForcold, nColdU, nColdA = 0, 0, 0, 0
    ...
    for i in range(serNum):
        ...
        hotRating = hotRating.drop(hotforcold.index)
        nHot += len(hotRating)
        nHotForcold += len(hotforcold)
        nColdU += len(coldRating['coldUser'])
        nColdA += len(coldRating['coldAD'])
        coldUserTrainData, coldUserTestData = get_train_test_data(coldRating['coldUser'])
        coldADTrainData, coldADTestData = get_train_test_data(coldRating['coldAD'])

        allColdTrain.append(pd.concat([coldUserTrainData, coldADTrainData]))
        hotTrainData, hotTestData = get_train_test_data(hotRating)

        sHotTrainData.append(hotTrainData)
        sHotTestData.append(hotTestData)
        sColdTrainData.append([coldUserTrainData, coldADTrainData])
        sColdTestData.append([coldUserTestData, coldADTestData])
    deviceTrainData, deviceServerID, deviceUserServerList = getdeviceData(sHotTrainData, sColdTrainData, use_cuda)
    for i in range(serNum):
        print(i, "hot data", ":", len(sHotTrainData[i]),
              "cold data" ":", len(sColdTrainData[i][0]), len(sColdTrainData[i][1]))

        sHotForColdData[i] = sda_dataloader(sHotForColdData[i], batch_size, use_cuda, rating_col)
        ...
        step = len(sHotForColdData[i])
        batchUser = np.math.ceil(len(sColdTrainData[i][0]) / step)
        batchAD = np.math.ceil(len(sColdTrainData[i][1]) / step)

        coldU = sda_dataloader(sColdTrainData[i][0], batchUser, use_cuda, rating_col)
        coldAD = sda_dataloader(sColdTrainData[i][1], batchAD, use_cuda, rating_col)
        print('!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!', step, len(coldU), len(coldAD))
        sColdTrainData[i] = [coldU, coldAD]
        coldU = sda_dataloader(sColdTestData[i][0], batch_size, use_cuda, rating_col)
        coldAD = sda_dataloader(sColdTestData[i][1], batch_size, use_cuda, rating_col)
        sColdTestData[i] = [coldU, coldAD]
    ...
    print("nHot", nHot, "nHotForcold", nHotForcold, "nColdU", nColdU, "nColdA", nColdA)
...
def eval_modle(nets, sHotTestData, sColdTestData):
    # 聚合之后测试模型
    aucHot = pd.DataFrame()
    aucColdUser = pd.DataFrame()
    aucColdAD = pd.DataFrame()
    for idx in range(len(nets)):
        net = nets[idx]
        hot = pd.DataFrame([net.evaluate(sHotTestData[idx])])
        coldUser = pd.DataFrame([net.evaluate(sColdTestData[idx][0])])
        coldAD = pd.DataFrame([net.evaluate(sColdTestData[idx][1])])
        aucHot = pd.concat([aucHot, hot], axis=0)
        aucColdUser = pd.concat([aucColdUser, coldUser], axis=0)
        aucColdAD = pd.concat([aucColdAD, coldAD], axis=0)
    print(" -------------------------------- hot: ")
    print(aucHot.mean(axis=0))
    print(" -------------------------------- cold user: ")
    print(aucColdUser.mean(axis=0))
    print(" -------------------------------- cold ad: ")
    print(aucColdAD.mean(axis=0))
    # return auc_single.mean(axis=0), auc_total.mean(axis=0)


def compileNet(net, lr, cli_idx):
    loss_func = net.net.lossFunc
    # params = filter(lambda p: p.requires_grad, net.net.parameters())
    optimizer = torch.optim.Adam(params=net.net.parameters(), lr=lr, weight_decay=0.001)
    metrics_dict = net.net.metricFunc
    net.compile(loss_func=loss_func, optimizer=optimizer, metrics_dict=metrics_dict, idx=cli_idx)


def trainNet(Epoch, nets, modleType, trainData, lr, best_nets, trainColIdx=None, printAuc=True, valid_data=None):
    global train_auc
    for epo in range(1, Epoch + 1):
        ...
        for idx in range(len(nets)):
            # net = compileNet(nets[idx], lr, idx)
            params = filter(lambda p: p.requires_grad, nets[idx].net.parameters())
            lossFunc = nets[idx].net.lossFunc
            metricFunc = nets[idx].net.metricFunc
            optimizer = torch.optim.Adam(params=params, lr=lr, weight_decay=0.002)
            nets[idx].compile(loss_func=lossFunc, optimizer=optimizer, metrics_dict=metricFunc, idx=idx + 1)
            # 聚合之前support,前向传播
            valid = valid_data if valid_data is None else valid_data[idx]
            met, loss = nets[idx].fitSDA(trainData[idx], valid_data=valid, trainColIdx=trainColIdx)
        ...
        if printAuc:
            print(" -------------------------------- train mean auc ", modleType, epo)
...
# def serverMetaTrain(nets, coldData1, coldData1):
#     # for i, net in enumerate(nets):
#     #     task_num = 0
#     #     net.fitSDA(coldData1[i])
#
...
def getdeviceData(sHotTrainData, sColdTrainData, use_cuda, batchSize=256):
    ...
    for hot, cold in zip(sHotTrainData, sColdTrainData):
        train = pd.concat([hot, cold[0], cold[1]], axis=0, ignore_index=True)
...
def deviceLocalTrain(localEpo, nets, deviceTrainData, deviceServerID, modleType, lr):
    deviceNets = getdeviceNets(nets, deviceServerID)
    trainCol = ['user_id', 'gender', 'age', 'occupation', 'zip_code']
    # setRequiresGradSomeTrue(deviceNets, trainCol)
    best_nets = {}
    for i in range(len(deviceNets)):
        best_nets[i] = [-100, None]
    deviceNets, _, meanAuc = trainNet(localEpo, deviceNets, modleType, deviceTrainData, lr, best_nets, None, False)
    return deviceNets, meanAuc
...
#
# def serverMetaTrain(nets, sHotForColdData, sColdTrainData, sColdTestData, lr):
#     update_step = 2
#     for i, kerasNet in enumerate(nets):
#         net = kerasNet.net
#         net.train()
#         for epo in range(1,6):
#             for hotData, coldData in zip(sHotForColdData[i], sColdTrainData[i]):
#                 # 1. run the i-th task and compute loss for k=0
#                 hotX, hotTure = hotData[0], hotData[1]
#                 coldX, coldTrue = coldData[0], coldData[1]
#                 hotPred = net(hotX)
#                 loss = net.lossFunc(hotPred, hotTure)
#                 #params = filter(lambda p: p.requires_grad, net.parameters())
#                 grad = torch.autograd.grad(loss, params, create_graph=True, retain_graph=True)
#                 fast_weights = list(map(lambda p: p[1] - lr * p[0], zip(grad, params)))
#                 for k in range(1, update_step):  # 第二步更新了
#                     updataFastWeight(net, fast_weights)
#                     hotPred = net(hotX)
#                     loss = net.lossFunc(hotPred, hotTure)
#                     params = filter(lambda p: p.requires_grad, net.parameters())
...
#                     coldPred = net(coldX)
#                     lossCold = net.lossFunc(coldPred, coldTrue)
#
#             kerasNet.optimizer.zero_grad()  # 梯度清零
#             lossCold.backward()
#             kerasNet.optimizer.step()
def initStateDict(dic, net):
    for name in net.state_dict().keys():
        dic[name] = torch.zeros_like(net.state_dict()[name].data)


def updataNetWeight(net, ori_theta):
    for name, weight in ori_theta.items():
        net.state_dict()[name].data.copy_(weight)


def getMetaGradient(metaGradient, net, theta=1.0):
    t = theta if theta == 1 else theta
    for name in net.state_dict().keys():
        metaGradient[name] += net.state_dict()[name].data
    return metaGradient


def serverMetaTrain_PnnDeepCopy(epoch, net, hot, cold, test, lr, serID):
    for epo in range(1, epoch + 1):
        # 对每一个任务进行迭代(训练),保留每一个任务梯度下降之后的参数
        dic = {}
        initStateDict(dic, net)
        global ori_theta, meta_gradient
        ori_theta, meta_gradient = dic, dic
        # 用联邦参数初始话网络
        fed_theta = copy.deepcopy({name: net.state_dict()[name].data for name in net.state_dict().keys()})
        ori_theta = copy.deepcopy(fed_theta)
        tasks = len(hot)
        aucSumHot, aucSumCold = {'auc': 0.0, 'loss': 0.0}, {'auc': 0.0, 'loss': 0.0}
        theta_matrix = [copy.deepcopy(dic) for _ in range(tasks)]
        for i, hotData in enumerate(list(hot)):
            hotX, hotY = hotData[0], hotData[1]
            updataNetWeight(net, ori_theta)
            auc, _ = net.train_step(hotX, hotY)
            for key, val in auc.items():
                aucSumHot[key] += val
            theta_matrix[i] = copy.deepcopy({name: net.state_dict()[name].data for name in net.state_dict().keys()})

        for i, coldtData in enumerate(list(cold[0])):
            coldX, coldY = coldtData[0], coldtData[1]
            updataNetWeight(net, theta_matrix[i])
            auc, _ = net.train_step(coldX, coldY)
            for key, val in auc.items():
                aucSumCold[key] += val
            meta_gradient = getMetaGradient(meta_gradient, net)
        ori_theta = {ori_theta[name] - lr * meta_gradient[name] / 2 / tasks for name in ori_theta.keys()}


def serverMetaTrain_UcatA(epoch, net, hot, cold, test, lr, serID):
    dic = {}
    initStateDict(dic, net)
    global ori_theta, meta_gradientU, meta_gradientV
    fed_theta = copy.deepcopy({name: net.state_dict()[name].data for name in net.state_dict().keys()})
    ori_theta = fed_theta
    meta_gradientU = copy.deepcopy(dic)
    meta_gradientV = copy.deepcopy(dic)
    tasks, taskU, taskA = len(hot), len(cold[0]), len(cold[0])
    theta_matrix = [dic for _ in range(tasks)]
    for epo in range(1, epoch + 1):
        # 对每一个任务进行迭代(训练),保留每一个任务梯度下降之后的参数
        for i, hotData in enumerate(list(hot)):
            hotX, hotY = hotData[0], hotData[1]
            updataNetWeight(net, ori_theta)
            auc, _ = net.train_step(hotX, hotY)
            theta = {name: net.state_dict()[name].data for name in net.state_dict().keys()}
            theta_matrix[i] = copy.deepcopy(theta)
        # 对每一个任务进行迭代（测试），利用保留的梯度下降之后的参数作为训练参数，计算梯度和
        task = 0
        for i, (coldU, coldA) in enumerate(zip(list(cold[0]), list(cold[1]))):
            coldX = torch.cat((coldU[0], coldA[0]))
            coldY = torch.cat((coldU[1], coldA[1]))
            updataNetWeight(net, theta_matrix[i])
            auc, _ = net.train_step(coldX, coldY)
            meta_gradientU = getMetaGradient(meta_gradientU, net)
            task += 1
        # 更新初始的ori_theta
        for name in ori_theta.keys():
            gradient = meta_gradientU[name] / task
            ori_theta[name] = 0.1 * ori_theta[name] - 0.9 * gradient


def serverMetaTrain_UA(epoch, net, hot, cold, test, lr, serID):
    for epo in range(1, epoch + 1):
        # 对每一个任务进行迭代(训练),保留每一个任务梯度下降之后的参数
        dic = {}
        initStateDict(dic, net)
        ...
        for i, coldtData in enumerate(list(cold[0])):
            ...
            meta_gradientU = getMetaGradient(meta_gradientU, net)
        # 对每一个任务进行迭代（测试），利用保留的梯度下降之后的参数作为训练参数，计算梯度和
        for i, coldtData in enumerate(list(cold[1])):
            coldX, coldY = coldtData[0], coldtData[1]
            updataNetWeight(net, theta_matrix[i])
            auc, _ = net.train_step(coldX, coldY)
            meta_gradientV = getMetaGradient(meta_gradientV, net)
        # 更新初始的ori_theta
        for name in ori_theta.keys():
            gradient = meta_gradientU[name] / taskU + meta_gradientV[name] / taskA
            ori_theta[name] = {ori_theta[name] - lr * gradient * 0.5}


# def serverMetaTrain_test(epoch, net, hot, cold, test, lr, serID):
#     dic = {}
#     initStateDict(dic, net)
#     global ori_theta, metaGradientUser, metaGradientAD, fedW
#     ori_theta, metaGradientUser, metaGradientAD = dic, dic, dic
#     # 用联邦参数初始话网络
#     metaTheta = copy.deepcopy({name: net.state_dict()[name].data for name in net.state_dict().keys()})
#     tasks = len(hot)
#     theta_matrix = []
#     for epo in range(1, epoch + 1):
#         # compileNet(net, lr, serID)
#         # 对每一个任务进行迭代(训练),保留每一个任务梯度下降之后的参数
#         aucSumHot, aucSumCold = {'auc': 0.0, 'loss': 0.0}, {'auc': 0.0, 'loss': 0.0}
#         for i, hotData in enumerate(list(hot)):
#             hotX, hotY = hotData[0], hotData[1]
#             updataNetWeight(net, metaTheta)
#             auc, _ = net.train_step(hotX, hotY)
#             for key, val in auc.items():
#                 aucSumHot[key] += val
#             theta = {name: net.state_dict()[name].data for name in net.state_dict().keys()}
#             theta_matrix.append(copy.deepcopy(theta))
#             #fedW = {name: net.state_dict()[name].data for name in net.state_dict().keys()}
#         # 对每一个coldUser进行迭代（测试），利用保留的梯度下降之后的参数作为训练参数，计算梯度和
#         for taksU, coldtData in enumerate(list(cold[0])):
#             coldX, coldY = coldtData[0], coldtData[1]
#             updataNetWeight(net, theta_matrix[taksU])
#             #updataNetWeight(net, fedW)
#             auc, _ = net.train_step(coldX, coldY)
#             for key, val in auc.items():
#                 aucSumCold[key] += val
#             metaGradientUser = getMetaGradient(metaGradientUser, net)
#         # 对每一个coldAD进行迭代（测试），利用保留的梯度下降之后的参数作为训练参数，计算梯度和
#         for taksA, coldtData in enumerate(list(cold[1])):
#             coldX, coldY = coldtData[0], coldtData[1]
#             updataNetWeight(net, theta_matrix[taksA])
#             #updataNetWeight(net, fedW)
#             auc, _ = net.train_step(coldX, coldY)
#             for key, val in auc.items():
#                 aucSumCold[key] += val
#             metaGradientAD = getMetaGradient(metaGradientAD, net)
#         # 更新初始的metaTheta
#         for name in metaTheta.keys():
#             metaTheta[name] = metaTheta[name] - lr * (metaGradientUser[name] / taksU + metaGradientAD[name] / taksA)


def sda_fed_torchkeras(Epoch, nets, modleType, fed_type, strategy, trainColIdx,
                       fea_dims, sHotTrainData, sHotTestData, sColdTrainData, sColdTestData, sHotForColdData,
                       deviceTrainData, deviceServerID, deviceUserServerList, serverDeviceID, aData,
                       use_cuda):
    print("---------------------------------------------------------------------------------", modleType, fed_type)
    ...
    # 联邦训练
    for epo in range(1, Epoch + 1):
        if fed_type in ['FedME', 'FedEmb']:
            print('-------------------------------- fed phrase', modleType, fed_type, epo)
            agg_item_emb(nets, aData, use_cuda, 'avg')
            adCol = ['movie_title', 'genre']
            average_in_cols(nets, adCol)
            deviceNets, meanAuc = deviceLocalTrain(3, nets, deviceTrainData, deviceServerID, modleType, lr)
            print(meanAuc)
            userCol = ['timestamp', 'gender', 'age', 'occupation', 'zip_code']
            serverAggregateUserEmb(nets, deviceNets, deviceUserServerList, serverDeviceID, userCol)
            print(" ------------------------------------------------ fed phrase done:")
            eval_modle(nets, sHotTestData, sColdTestData)

        if fed_type == 'FedEmb':
            print(" --------------------------------", modleType, fed_type, epo)
            coldUser = [tmp[0] for tmp in sColdTrainData]
            coldAD = [tmp[1] for tmp in sColdTrainData]
            nets, _, _ = trainNet(3, nets, modleType, coldUser, lr, best_nets, None, True)
            nets, _, _ = trainNet(3, nets, modleType, coldAD, lr, best_nets, None, True)
            print(" ------------------------------------------------ eval modle:", modleType, fed_type, epo)
            eval_modle(nets, sHotTestData, sColdTestData)

        if fed_type in ['FedME', 'MetaEmb']:
            print(" ------------------------------------------------ meta phase", modleType, fed_type, epo)
            setRequiresGradAllTrue(nets)
            for i, (net, hot, cold, test) in enumerate(zip(nets, sHotForColdData, sColdTrainData, sColdTestData)):
                serverMetaTrain_UcatA(3, net, hot, cold, test, lr, i + 1)
            print(" ---------------------------------------------------- meta phrase done:")
            eval_modle(nets, sHotTestData, sColdTestData)
        ...
        if fed_type == 'RandEMb':
            ...
            coldUser = [tmp[0] for tmp in sColdTrainData]
            coldAD = [tmp[1] for tmp in sColdTrainData]
            nets, _, _ = trainNet(1, nets, modleType, coldUser, lr, best_nets, None, True)
            nets, _, _ = trainNet(1, nets, modleType, coldAD, lr, best_nets, None, True)
            print(" ------------------------------------------------  eval modle:", modleType, fed_type, epo)
            eval_modle(nets, sHotTestData, sColdTestData)

    print(" --------------------------------------------------------- done test:", modleType, fed_type)
...
def preTrain(Epoch, modleType, sHotTrainData, sHotForColdData, sHotTestData, sColdTestData, fea_dims, trainColIdx,
    ...
    _, best_nets, _ = trainNet(Epoch, nets, modleType + ' pretrain', sHotTrainData, lr, best_nets,
                               trainColIdx, printAuc=True, valid_data=sHotForColdData)
    nets = [best_nets[i][1] for i in range(len(best_nets))]
    del best_nets
    eval_modle(nets, sHotTestData, sColdTestData)
...
def loadPreTrain(modleType, sHotTestData, sColdTestData, fea_dims, use_cuda):
    ...
    print("\n------------------------------------------------load pretrain done, eval", modleType)
    eval_modle(preNet, sHotTestData, sColdTestData)
    return preNet
...
def train_sda():
    ...
    fea_dims, sHotTrainData, sHotTestData, sColdTrainData, sColdTestData, sHotForColdData, \
    deviceTrainData, deviceServerID, deviceUserServerList, serverDeviceID, aData = \
        readdata_SDA_ML1M(path, serNum, batch_size=256, test=test)

    modles = ['mlp', 'pnn', 'dcn', 'metamf', 'mynet']
    fed_types = ["RandEMb", "FedEmb", "MetaEmb", "FedME"]  # "Central",
    fed_types = ["FedME"]  # "Central",
    strategy = {'u': 'my_pca', 'v': 'my_pca'}
    # ['user_id', 'movie_id', 'movie_title', 'genre', 'timestamp', 'gender', 'age', 'occupation', 'zip_code', 'rating']

    trainCol = ['user_id', 'movie_id', 'genre', 'movie_title', 'timestamp']
    ...
    for modleType in modles[1:3]:
        ...
        for fed_type in fed_types:
            nets = loadPreTrain(modleType, sHotTestData, sColdTestData, fea_dims, use_cuda)
            sda_fed_torchkeras(Epoch, nets, modleType, fed_type, strategy, trainColIdx,
                               fea_dims, sHotTrainData, sHotTestData, sColdTrainData, sColdTestData, sHotForColdData,
                               deviceTrainData, deviceServerID, deviceUserServerList, serverDeviceID, aData,
                               use_cuda)
