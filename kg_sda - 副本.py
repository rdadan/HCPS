...
def getColdAndHotRating(df_rating, minClodU=40, minClodA=10):
    total = len(df_rating)
    ad = df_rating['movie_id'].value_counts().to_frame()
    coldAd = ad.loc[ad.values < minClodA].index.tolist()
    coldAdRating = df_rating.loc[df_rating['movie_id'].isin(coldAd)]

    user = df_rating['user_id'].value_counts().to_frame()
    coldUserID = user.loc[user.values < minClodU].index.tolist()
...
def readdata_SDA_ML1M_ALLCol(root_path, serNum, batch_size, use_cuda=False, test=False):
    ...
    for i in range(serNum):
        ...
        numHotIDs += (hotRating['user_id'].nunique()+hotRating['movie_id'].nunique())
        numColdIDs += (coldRating['user_id'].nunique()+coldRating['movie_id'].nunique())
        coldTrainData, coldTestData = get_train_test_data(coldRating)
        hotTrainData, hotTestData = get_train_test_data(hotRating)
        hotforcold = hotTrainData[:len(coldTrainData)]
    ...
    deviceTrainData, deviceServerID, deviceUserServerList = getdeviceData(rating_col, sHotTrainData, sColdTrainData,
                                                                          use_cuda)
    del serverUserID
    for i in range(serNum):
        print(i, "hot data", len(sHotTrainData[i]), "cold data", len(sColdTrainData[i]), "hotforcod data",
              len(sHotForColdData[i]))
        ...
        print("hot step", ":", len(sHotTrainData[i]), "hotcold step", ":", len(sHotForColdData[i]), "cold step" ":",
              len(sColdTrainData[i]))
    ...
    print('numHotIDs:', numHotIDs, "numCotIDs:", numColdIDs)
...
def readdata_SDA_ML1M_Central(root_path, serNum, batch_size, use_cuda=False, test=False):
    # 全部数据
    rating_col = ['user_id', 'movie_id', 'movie_title', 'genre', 'timestamp', 'gender',
                  'age', 'occupation', 'zip_code', 'rating']
    ...
    coldRating, hotRating = getColdAndHotRating(df, 60, 30)
    coldTrain, coldTest = get_train_test_data(coldRating)
    hotTrain, hotTest = get_train_test_data(hotRating)
    hotforcold = hotRating[:len(hotTrain)]

    coldTrain = sda_dataloader(coldTrain, batch_size, use_cuda, rating_col)
    coldTest = sda_dataloader(coldTest, batch_size, use_cuda, rating_col)
    hotTrain = sda_dataloader(hotTrain, batch_size, use_cuda, rating_col)
    hotTest = sda_dataloader(hotTest, batch_size, use_cuda, rating_col)
    hotforcold = sda_dataloader(hotforcold, batch_size, use_cuda, rating_col)

    return coldTrain, coldTest, hotTrain, hotTest, hotforcold
...
def sda_init_net(cli_nums, fea_dims, modletype, use_cuda):
    ...
    for idx in range(cli_nums):
        ...
        if modletype == 'mlp':
            ...
            net = DCN(fea_dims, hiddenUnits, 1, use_cuda, lossname, embDimDict)
        else:
...
def eval_modle(nets, sHotTestData, sColdTestData, trainColIdx=None, history=None):
    # 聚合之后测试模型
    aucHot = pd.DataFrame()
    ...
    hotMean = aucHot.mean(axis=0)
    print(hotMean)
    print(" -------------------------------- mean on cold data: ")
    coldMean = aucCold.mean(axis=0)
    print(coldMean)
    if history is not None:
        history.loc[len(history.index)] = [hotMean['val_loss'], hotMean['val_auc'], coldMean['val_loss'],
                                           coldMean['val_auc']]
...
def trainNet(Epoch, nets, modleType, trainData, lr, best_nets, trainColIdx=None, printAuc=True, validData=None):
    ...
    for epo in range(1, Epoch + 1):
        ...
        for idx in range(len(nets)):
            ...
            optimizer = torch.optim.Adam(params=nets[idx].net.parameters(), lr=lr, weight_decay=0.0001)
...
def getdeviceData(rating_col, sHotTrainData, sColdTrainData, use_cuda, batchSize=256):
    ...
    for hot, cold in zip(sHotTrainData, sColdTrainData):
        ...
        serverUserID.append(train['user_id'].unique())
...
def updataNetWeight(net, oriTheta):
    for name, weight in oriTheta.items():
        net.state_dict()[name].data = copy.deepcopy(weight.data)
...
def sda_fed_torchkeras(Epoch, nets, modleType, fed_type, trainColIdx,
    ...
    # 联邦训练
    history = {'hot_loss': [], 'hot_auc': [], 'cold_loss': [], 'cold_auc': []}
    fedHistory = pd.DataFrame(history)
    metaHistory = pd.DataFrame(history)
    eval_modle(nets, sHotTestData, sColdTestData, history=fedHistory)
    eval_modle(nets, sHotTestData, sColdTestData, history=metaHistory)
    for epo in range(1, Epoch + 1):
        ...
        if fed_type == 'MetaEmb':
            ...
            setRequiresGradAllTrue(nets)
            for i, (net, hot, cold, test) in enumerate(zip(nets, sHotForColdData, sColdTrainData, sColdTestData)):
                serverMetaTrainDeepCopy(3, net, hot, cold, test, lr, i + 1)
            eval_modle(nets, sHotTestData, sColdTestData)
        if fed_type == 'FedAvg':
    ...
    eval_modle(nets, sHotTestData, sColdTestData)
    print(" -------------------------------- save history log done", modleType, fed_type)
    name0 = "fed" + modleType + fed_type + '.csv'
    name1 = "meta" + modleType + fed_type + '.csv'
    fedHistory.to_csv(name0)
    metaHistory.to_csv(name1)
...
def train_sda():
    ...
    #path_pc = '/home/stu1/rxj/sda/dataset/da/allcol mixture distribution 10-10/'
