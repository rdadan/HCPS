...
def readdata_SDA_ML1M(root_path, serNum, batch_size, use_cuda=False, test=False):
    ...
    for i in range(serNum):
        ...
        setClickFlag([data])
        getServerData(data, serverDeviceID, sHotForColdData, sHotTrainData, sHotTestData, sColdTrainData, sColdTestData)
    ...
    for i in range(serNum):
        ...
        sHotForColdData[i] = sda_dataloader(sHotForColdData[i], bach, use_cuda, rating_col[:-1])
        sHotTrainData[i] = sda_dataloader(sHotTrainData[i], batch_size, use_cuda, rating_col[:-1])
        sHotTestData[i] = sda_dataloader(sHotTestData[i], batch_size, use_cuda, rating_col[:-1])
        sColdTrainData[i] = sda_dataloader(sColdTrainData[i], batch_size, use_cuda, rating_col[:-1])
        sColdTestData[i] = sda_dataloader(sColdTestData[i], batch_size, use_cuda, rating_col[:-1])
...
def eval_modle(nets, sHotTestData, sColdTestData):
    ...
    for idx in range(len(nets)):
        ...
        cold = pd.DataFrame([net.evaluate(sColdTestData[idx])])
...
def trainNet(Epoch, nets, modleType, trainData, lr, best_nets, trainColIdx=None, printAuc=True):
    global train_auc
    for epo in range(1, Epoch + 1):
        ...
        for idx in range(len(nets)):
            ...
            # 聚合之前support,前向传播
            met, loss = net.fitSDA(trainData[idx], trainColIdx=trainColIdx)
            # if printAuc:
            #     print(met)
            train_auc = pd.concat([train_auc, met], axis=0)
            # 保存模型
            if printAuc and epo % 1 == 0 and float("%.4f" % met['auc']) > best_nets[idx][0]:
...
def getdeviceData(sHotTrainData, sColdTrainData, use_cuda, batchSize=256):
    ...
    for data in deviceData:
        ...
        deviceTrainData.append(sda_dataloader(data, batchSize, use_cuda, rating_col[:-1]))
...
def deviceLocalTrain(nets, deviceTrainData, deviceServerID, modleType, lr):
    deviceNets = getdeviceNets(nets, deviceServerID)
    trainCol = ['user_id', 'age', 'gender', 'occupation']
    setRequiresGradSomeTrue(deviceNets, trainCol)
    best_nets = {}
    for i in range(len(deviceNets)):
        best_nets[i] = [-100, None]
    deviceNets, _, _ = trainNet(3, deviceNets, modleType, deviceTrainData, lr, best_nets, None, True)
    return deviceNets
...
def getMetaGradient(metaGradient, net):
    for name in net.state_dict().keys():
        metaGradient[name] += net.state_dict()[name].data
    return metaGradient


def serverMetaTrain(epoch, net, hot, cold, test, lr, serID):
    for epo in range(1, epoch + 1):
        ...
        # 用联邦参数初始话网络
        fed_theta = {name: net.state_dict()[name].data for name in net.state_dict().keys()}
        ...
        # 更新初始的ori_theta
        ori_theta = {ori_theta[name] - lr * meta_gradient[name] / tasks for name in ori_theta.keys()}
...
def sda_fed_torchkeras(Epoch, nets, modleType, fed_type, strategy, trainColIdx,
    ...
    for epo in range(1, Epoch + 1):
        ...
        if fed_type == 'AggEmbAvg':
            # 聚合ad
            print('-------------------------------- advertiser aggregate ad emb', modleType, fed_type, epo)
            agg_item_emb(nets, aData, use_cuda, 'avg')
            average_in_cols(nets, ['genre'])
            # 训练 user
            print('-------------------------------- device local train...', modleType, fed_type, epo)
            deviceNets = deviceLocalTrain(nets, deviceTrainData, deviceServerID, modleType, lr)
            # 聚合
            print('-------------------------------- server aggregate user embedding...')
            feaName = ['user_id', 'age', 'gender', 'occupation']
            # serverAggregateUserParameter(nets, serverUserID, dData, idParemeter, feasParemeter, feaName)
            serverAggregateUserEmb(nets, deviceNets, deviceUserServerList, serverDeviceID, feaName)
            # 聚合测试
            print(" -------------------------------- after fed server evalue:")
            eval_modle(nets, sHotTestData, sColdTestData)

            print(train_auc.mean(axis=0))
        if fed_type == 'FedAvg':
            print(" -------------------------------- fed aggregate", modleType, fed_type, epo)
            # aggregateNet(nets, dData, aData, fed_type, use_cuda, strategy)
            average_all_layers(nets)
            print(" -------------------------------- eval after fed", modleType, fed_type, epo)
            eval_modle(nets, sHotTestData, sColdTestData)

        # 服务器训练
        # if epo != Epoch:
        print(" -------------------------------- server train...", modleType, fed_type, epo)
        trainCol = ['user_id', 'movie_id', 'age', 'gender', 'occupation', 'genre']
        setRequiresGradAllTrue(nets)
        for i, (net, hot, cold, test) in enumerate(zip(nets, sHotForColdData, sColdTrainData, sColdTestData)):
            serverMetaTrain(3, net, hot, cold, test, lr, i+1)
        # nets, best_nets, train_auc_mean = trainNet(1, nets, modleType, sHotForColdData,
        #                                            lr, best_nets, trainColIdx, printAuc=True)
        # setRequiresGradAllTrue(nets)
        # nets, best_nets, train_auc_mean = trainNet(1, nets, modleType, sColdTrainData,
        #                                            lr, best_nets, trainColIdx, printAuc=True)
        eval_modle(nets, sHotTestData, sColdTestData)
...
def preTrain(Epoch, modleType, sHotTrainData, sHotTestData, sColdTestData, fea_dims, trainColIdx, use_cuda):
    # server 预训练
    lr = 0.001
    best_nets = {}
    cli_nums = len(sHotTrainData)
...
def train_sda():
    ...
    use_cuda = False
    path_pc = '../input/user-ad-cold-mixture-distribution-1010/'
    path, Epoch, test, auc_log_s, auc_log_t = path_pc, 31, False, {}, {}
    ...
    fea_dims, sHotTrainData, sHotTestData, sColdTrainData, sColdTestData, sHotForColdData, \
    deviceTrainData, deviceServerID, deviceUserServerList, serverDeviceID, aData = \
        readdata_SDA_ML1M(path, serNum, batch_size=300, test=test)

    modles = ['mlp', 'pnn', 'dcn', 'metamf', 'mynet']
    fed_types = ["FedAvg", "AggEmbAvg"]  # "Central",
    strategy = {'u': 'my_pca', 'v': 'my_pca'}
    trainColIdx = getTrainFeaIdx(fea_dims)
    for modleType in modles[2:3]:
        preTrain(Epoch, modleType, sHotTrainData, sHotTestData, sColdTestData, fea_dims, trainColIdx, use_cuda)
        for fed_type in fed_types[1:2]:
